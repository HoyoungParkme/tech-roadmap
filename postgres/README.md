...
두꺼운 책 없이, 클릭 한 번으로 PostgreSQL의 핵심 개념을 눈으로 확인하며 익히는 학습 자료다.

---

## 이 자료의 목적

주니어 개발자가 PostgreSQL 코드베이스와 SQL 쿼리를 처음 접했을 때 가장 먼저 부딪히는 개념들 — SQL 실행 파이프라인, 인덱스 원리, 트랜잭션 ACID, 실행 계획(EXPLAIN) — 을 **인터랙티브 시각화**로 빠르게 체득하는 것이 목표다.

책을 읽으면 이해하는 데 며칠이 걸리는 내용을, 애니메이션을 보고 직접 조작하면서 수 시간 안에 머릿속에 그림이 그려지도록 설계했다. 이 자료를 전부 익히고 나면 실무 PostgreSQL 쿼리를 보고 "왜 이렇게 짰는지"가 자연스럽게 보이기 시작한다.
...
VSCode에서 이 `postgres/` 폴더를 연다. 파일 탐색기에서 `index.html`을 찾아 **우클릭** → **Open with Live Server**를 선택한다.

기본 브라우저에서 `http://127.0.0.1:5500/postgres/index.html` (또는 유사한 주소)로 자동으로 열린다.

---

## GitHub Pages에 배포하기

### 방법 A: `postgres/` 폴더 전체를 새 레포 루트로 (권장)

이 방법이 가장 단순하다. `index.html`이 레포 루트에 위치해야 GitHub Pages가 자동으로 진입점을 인식한다.

```bash
# postgres/ 폴더 내용을 새 레포에 올리는 예시
cd postgres/
git init
git add .
git commit -m "feat: Chapter IV-1-1 SQL 쿼리 실행 파이프라인 시각화"
...
`postgres/` 폴더를 기존 레포 안에 그대로 push하면 된다. 이 경우 GitHub Pages Source를 `/docs`로 지정하는 기능이 필요하거나, 별도 브랜치(예: `gh-pages`)를 써야 할 수 있다. 구조가 복잡해지므로 방법 A를 권장한다.
...
현재 `index.html`은 **Chapter IV-1-1 (SQL 쿼리 실행 파이프라인)** 시각화다. 클라이언트가 보낸 `SELECT * FROM users WHERE age > 25`가 Parser → Planner/Optimizer → Executor → Storage Manager를 거쳐 결과 행으로 반환되기까지의 6단계 과정을 단계별 애니메이션으로 보여준다.

| Part | 제목 | 대상 난이도 | 시각화 |
|:--|:--|:--|:--|
| I | 왜 DB가 필요한가 | ★☆☆ (비개발자) | — |
| II | RDB 개론 (MySQL/Oracle 비교) | ★☆☆~★★☆ | — |
| III | PostgreSQL 첫 걸음 | ★★☆ (코딩 입문) | — |
| IV | SQL 코어 개념 | ★★☆~★★★ | ✅ (`index.html` = IV-1-1 쿼리 실행 파이프라인) |
| V | 트랜잭션·인덱스·성능 | ★★★ (주니어) | — |
| VI | 실전 스키마 설계 | ★★★ | — |
| VII | 복제·백업·모니터링 | ★★★ | — |
...
├── chapter-iv-01-01-pipeline.html      # 현재 시각화
├── chapter-iv-01-02-explain.html       # 향후 추가
└── curriculum.md
```
...
새 챕터에서 SQL 예시를 바꿀 때는 `SQL_CODE` 배열만 교체하면 된다.
...
SQL 구문 강조(highlighting)는 `SQL_KEYWORDS` Set에 키워드를 추가하면 자동 적용된다.
...
### SQL 코드가 코드 뷰어에서 하이라이트되지 않는다

`SQL_KEYWORDS` Set에 해당 키워드가 포함되어 있는지 확인한다. 키워드 비교는 대소문자를 무시(`toUpperCase()`)하므로 대문자로 추가하면 된다. `--` 주석이 하이라이트되지 않는 경우에는 `commentIdx` 로직이 해당 줄에서 `--`를 찾고 있는지 점검한다.

### 단계 전환이 너무 빠르거나 느리다

`useEffect` 내 `setInterval`의 인터벌 값(기본 2500ms)을 조정한다.

```javascript
playIntervalRef.current = setInterval(() => handleNextRef.current(), 2500); // ms 단위
```
