# Spring Boot 인터랙티브 마스터 코스

두꺼운 책 없이, 클릭 한 번으로 Spring Boot의 핵심 개념을 눈으로 확인하며 익히는 학습 자료다.

---

## 이 자료의 목적

주니어 개발자가 Spring Boot 코드베이스에 처음 투입됐을 때 가장 먼저 부딪히는 개념들 — HTTP 요청 처리 계층, 의존성 주입(DI), JPA/Hibernate, 3계층 아키텍처 — 을 **인터랙티브 시각화**로 빠르게 체득하는 것이 목표다.

책을 읽으면 이해하는 데 며칠이 걸리는 내용을, 애니메이션을 보고 직접 조작하면서 수 시간 안에 머릿속에 그림이 그려지도록 설계했다. 이 자료를 전부 익히고 나면 실무 Spring Boot 코드를 보고 "왜 이렇게 짰는지"가 자연스럽게 보이기 시작한다.
...
VSCode에서 이 `springboot/` 폴더를 연다. 파일 탐색기에서 `index.html`을 찾아 **우클릭** → **Open with Live Server**를 선택한다.

기본 브라우저에서 `http://127.0.0.1:5500/springboot/index.html` (또는 유사한 주소)로 자동으로 열린다.

`index.html`은 챕터 허브 페이지다. 페이지에 표시된 챕터 카드 중 하나를 클릭하면 해당 시각화 파일로 이동한다.

---

## GitHub Pages에 배포하기

### 방법 A: `springboot/` 폴더 전체를 새 레포 루트로 (권장)

이 방법이 가장 단순하다. `index.html`이 레포 루트에 위치해야 GitHub Pages가 자동으로 진입점을 인식한다.

```bash
# springboot/ 폴더 내용을 새 레포에 올리는 예시
cd springboot/
git init
git add .
git commit -m "feat: Chapter 1-1 HTTP 요청 처리 계층 시각화"
...
`springboot/` 폴더를 기존 레포 안에 그대로 push하면 된다. 이 경우 GitHub Pages Source를 `/docs`로 지정하는 기능이 필요하거나, 별도 브랜치(예: `gh-pages`)를 써야 할 수 있다. 구조가 복잡해지므로 방법 A를 권장한다.
...
`index.html`은 챕터 허브(목차) 페이지다. 아래 표의 시각화 챕터들을 카드 목록으로 모아 두고, 원하는 챕터 카드를 클릭하면 해당 HTML 파일로 이동한다.

| Part | 제목 | 대상 난이도 | 시각화 |
|:--|:--|:--|:--|
| I | 왜 Java 프레임워크인가 | ★☆☆ (비개발자) | — |
| II | JVM·Spring 생태계 | ★☆☆~★★☆ | — |
| III | Spring Boot 첫 걸음 | ★★☆ (코딩 입문) | — |
| IV | Spring Boot 코어 (DI/AOP) | ★★☆~★★★ | ✅ IV-1-1 HTTP 요청 처리 계층 (`chapter-04-01-request-flow.html`), ✅ IV-2-1 Bean 생명주기 (`chapter-04-02-bean-lifecycle.html`) |
| V | 웹·DB·보안 계층 | ★★★ (주니어) | — |
| VI | 실전 REST API 프로젝트 | ★★★ | — |
| VII | 배포·로깅·모니터링 | ★★★ | — |
...
Part I~II는 코딩 경험이 없어도 막히지 않도록 비개발자 입문(★☆☆) 톤으로 본문이 작성되어 있다. 각 Section 설명이 평균 16~24줄 분량의 평어체 본문으로 구성되어 있어, Java나 프레임워크를 처음 접하는 독자도 Part I부터 순서대로 읽으면 된다.
...
```
springboot/
├── index.html                          # 챕터 허브 (목차 페이지)
├── chapter-04-01-request-flow.html     # Chapter IV-1-1 HTTP 요청 처리 계층
├── chapter-04-02-bean-lifecycle.html   # Chapter IV-2-1 Bean 생명주기
├── curriculum.md                       # Part I~VII 전체 목차 + 본문
└── README.md
```

새 챕터를 추가할 때는 아래 절차를 따른다.

1. `chapter-XX-YY-이름.html` 파일을 새로 만든다 (XX = Part 번호, YY = Chapter 번호).
2. `index.html`의 챕터 카드 섹션에 새 파일을 가리키는 카드를 하나 추가한다.
3. `curriculum.md`의 해당 Section "시각화 연결" 필드를 업데이트한다.
