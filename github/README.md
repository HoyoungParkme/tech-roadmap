# Git & GitHub 인터랙티브 마스터 코스

두꺼운 책 없이, 클릭 한 번으로 Git의 핵심 개념을 눈으로 확인하며 익히는 학습 자료다.

---

## 이 자료의 목적

주니어 개발자가 처음 팀 프로젝트에 투입됐을 때 가장 먼저 부딪히는 것들 — 브랜치 전략, 머지 충돌, PR 리뷰 흐름, CI/CD 자동화 — 을 **인터랙티브 시각화**로 빠르게 체득하는 것이 목표다.

`report_최종_진짜최종.docx` 방식으로 버전 관리를 해본 경험이 있다면, Part I부터 읽어도 막히지 않게 만들었다. 이 자료를 전부 익히고 나면 팀의 Git 워크플로우를 보고 "왜 이렇게 쓰는지"가 자연스럽게 보이기 시작한다.
...
VSCode에서 이 `github/` 폴더를 연다. 파일 탐색기에서 `index.html`을 찾아 **우클릭** → **Open with Live Server**를 선택한다.

기본 브라우저에서 `http://127.0.0.1:5500/github/index.html` (또는 유사한 주소)로 자동으로 열린다.

---

## GitHub Pages에 배포하기

### 방법 A: `github/` 폴더 전체를 새 레포 루트로 (권장)

이 방법이 가장 단순하다. `index.html`이 레포 루트에 위치해야 GitHub Pages가 자동으로 진입점을 인식한다.

```bash
# github/ 폴더 내용을 새 레포에 올리는 예시
cd github/
git init
git add .
git commit -m "feat: Chapter IV-1-1 Git 변경사항 여행 시각화"
...
`github/` 폴더를 기존 레포 안에 그대로 push하면 된다. 이 경우 GitHub Pages Source를 `/docs`로 지정하는 기능이 필요하거나, 별도 브랜치(예: `gh-pages`)를 써야 할 수 있다. 구조가 복잡해지므로 방법 A를 권장한다.
...
현재 루트 `index.html`은 **Chapter IV-1-1 (Git 변경사항의 여행)** 시각화다. 파일을 수정한 순간부터 팀 코드베이스에 Merge되기까지 Working Directory → Staging Area → Local Repository → Remote → PR/Review → Merge 6단계를 단계별 애니메이션으로 보여준다.
...
| III | 깃 첫 걸음 | ★★☆ (코딩 입문) | — |
| IV | 브랜치·머지·리베이스 | ★★☆~★★★ | ✅ (`index.html` = IV-1-1 Git 변경사항 여행) |
| V | PR·리뷰·Issue | ★★★ (주니어) | — |
| VI | 실전 팀 워크플로우 | ★★★ | — |
...
├── chapter-iv-01-journey.html        # 현재 index.html 내용
├── chapter-iv-02-branch.html
└── curriculum.md
```
